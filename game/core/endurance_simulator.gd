...
func configure(grid: Dictionary, duration: float, seed: int = 24) -> void:
	...
	for class_name in ["HYPERCAR", "PROTOTYPE", "GT3"]:
		for i in int(grid.get(class_name, 0)):
			var strategy := ["FUEL SAVE", "BALANCED", "PUSH"][i % 3]
			cars.append({
				"id": "%s_%02d" % [class_name, i + 1], "class": class_name,
...
func _simulate_car(car: Dictionary, step: float) -> void:
	...
	var traffic := _traffic_factor(car)
...
func _update_positions() -> void:
	...
	for class_name in CLASS_PACE:
		var group := cars.filter(func(car: Dictionary) -> bool: return car.class == class_name)

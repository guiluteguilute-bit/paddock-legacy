...
func _init() -> void:
	...
	assert(state.data.save_version==5);assert(state.career_config.kart_calendar.size()==6);assert(state.career_config.rivals.size()==11);assert(state.points_for(1)==25 and state.points_for(10)==1 and state.points_for(11)==0)
	...
	var endurance=load("res://game/core/endurance_simulator.gd").new();endurance.configure({"HYPERCAR":8,"PROTOTYPE":8,"GT3":16},360.0,77);assert(endurance.cars.size()==32);endurance.simulate(180.0);var gt=endurance.get_car("GT3_01");assert(gt.overall_position>0 and gt.class_position>0 and gt.drivers.size()==3);var stop=endurance.pit("GT3_01",{"fuel":true,"tyres":false,"driver_change":true,"repairs":"QUICK"});assert(stop>40.0 and gt.driver_index==1 and gt.fuel_laps==12.0);assert(endurance.apply_damage("GT3_01","AERO",25.0));assert(gt.condition.AERO==75.0);assert(endurance.set_time_scale(16) and not endurance.set_time_scale(3));var before=endurance.elapsed_minutes;assert(endurance.simulate_to_next_event()>0.0 and endurance.elapsed_minutes>before)
	state.endurance_config=state.load_endurance_config();state.data.reputation=100;assert(state.can_enter_endurance_class("HYPERCAR"));assert(state.complete_endurance_event("legacy_24",4,1,"HYPERCAR"));assert("24H WINNER" in state.data.endurance_trophies and state.data.prestige==100)

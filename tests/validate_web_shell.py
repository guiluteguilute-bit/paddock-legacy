...
assert re.search(r'#game\s*\{[^}]*position\s*:\s*fixed[^}]*top\s*:\s*var\(--app-top', shell, re.DOTALL), "#game must follow the visible viewport offset"
...
assert "visualViewport.addEventListener('scroll'" in shell, "Safari toolbar offset changes must update the canvas"
...
assert "new URL('build-version.json', rootUrl)" in shell and "cache_bust" in shell and "cache: 'no-store'" in shell, "Version polling must bypass caches"
assert "new URL('../../', window.location.href)" in shell, "Release shell must resolve the portable site root"
assert "window.location.replace(destination.href)" in shell, "Updates must return through the root launcher"
